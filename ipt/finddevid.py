from PyQt5.QtWidgets import (QTextEdit,QAction, qApp,QApplication,QMainWindow,QMessageBox,QDateTimeEdit,QComboBox,QFileDialog,QPushButton,QProgressBar,QGridLayout,QApplication, QCheckBox, QDialog,
        QDialogButtonBox, QFrame, QGroupBox, QLabel, QLineEdit, QListWidget,QComboBox,
...
class TabDialog(QDialog):           ##########主界面
    ...
    def creatui(self):
        ...
        self.permissionsLayout.addWidget(self.jsComboBox,2,0,1,1)
        ...
        self.permissionsLayout.addWidget(self.le_devname,2,1,1,7)
    ...
    def work(self):
        ...
        mode=self.jsComboBox.currentText()
        if mode=='设备名':
            ...
            tx=self.ts.main1(self.le_devname.text())
            self.reviewEdit.setPlainText(tx)
            self.qtb1.setEnabled(True)
            QMessageBox.information(self, "信息", "查询成功",QMessageBox.Yes)
    def saveconfig(self):
...
class zldatbase():
    ...
    def main1(self,id2):
        res=self.id_get_name(id2)
        upid=self.checkcont(id2)
        #print(upid,123)
        upname=self.id_get_name(upid)
        return '上级设备名:{},ID{}|设备名:{},ID:{}'.format(upname,upid,res,id2)
