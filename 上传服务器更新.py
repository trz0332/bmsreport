...
import requests
try:
    from ipt.zjyaml import *
    from ipt.encryption import saveyaml as en_saveyaml
    from ipt.encryption import loadyaml as en_loadyaml
    path = './ipt/'
except:
    from zjyaml import *
    ...
    for root, dirs, files in os.walk(path2):
        ...
        for i in files:
            if  i.split('.')[-1]  in  ['pyc']  and i not in  e_file:
                os.rename(path2 + i, (path2 + i).replace('.cpython-37', ''))  
    ###############上传函数
